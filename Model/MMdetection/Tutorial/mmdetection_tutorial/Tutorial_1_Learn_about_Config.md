...
- module을 분해하고 계층적인으로 디자인한다.
    - **Modify config through script arguments**
        - config file 내부 수정
            - dict의 key 순서에 따라 변경 가능하다.
    ...
    - **Config File Structure**
        - config/_base_ 아래에 4가지 basic 구성 요소가 존재한다
            1. dataset
            2. model
            3. schedule
            4. dafault_runtime
            
            - _base_의 요소들로 이루어진 config를 ***primitive***라고 부른다.
            - 같은 folder내에 config들은 하나의 primitive config를 가지기를 권장 다른 config의 경우 최대 상속은 3으로 하는게 좋다.
    - **Config Name Style**
        
        ```python
        {model}_[model setting]_{backbone}_{neck}_[norm setting]_[misc]_\
        [gpu x batch_per_gpu]_{schedule}_{dataset}
        ```
        
        {xxx} 필수, [yyy] 옵션
        
        - model: model type → ex) fater_rcnn
        - model setting: 특정 setting → ex) without_semantic
        - backbone: backbone type → ex) r50
        - neck : neck type → ex) fpn
        - norm_setting : norm layer → ex) bn, gn ,syncbn
        - misc : miscellaneous setting(잡다한 세팅) → ex) dconv,albu …
        - gpu x batch_per_gpu : GPUs and samples per GPU → ex) 8x2 is default
        - schedule : training scahedule option is 1x,2x,20e
            - nx → n*12 epochs → lr 1/10 씩 준다 8/16 , 11/22에서
            - ne → n epochs → lr 1/10 씩 준다 16 , 19 에서
        - dataset : dataset → ex) coco,voc…
    ...
    - **FAQ**
        - Ignore some fields in the base configs
            - old field 지우기 위해서는 -delte_ = True로 주고 새로운 field값을 준다.
            - 아래의 예시는 backbone의 모든 old key값들을 새로운 값으로 변환한다.
        ...
        - Use intermediate variables in configs
            - config file의 중간의 변수 사용 & 수정 방법
            - 아래의 예시는 mask rcnn train_pipeline/test_pipeline 수정 과정
        
        ```python
        # 새로운 train/test pipe line 정의
